...
- **音效与背景音乐全部由 Web Audio 现场合成**，不加载任何外部音频文件：落子/吃子音色不同、将军会喊出来（浏览器语音合成）、五声音阶古风循环 BGM
- **可换成你自己的音频**：把文件按下面的名字放到 `xiangqi/index.html` 同目录即可自动生效，找不到就回落合成音
  | 文件名 | 用途 |
  |---|---|
  | `bgm.mp3` | 背景音乐 |
  | `sfx-move.mp3` | 落子 |
  | `sfx-capture.mp3` | 吃子 |
  | `sfx-check.mp3` | 将军 |
  | `sfx-win.mp3` / `sfx-lose.mp3` | 胜利 / 失败 |

  也可以在界面里点「📂 换背景音乐」从设备选一个文件（存在浏览器 IndexedDB，只在本机播放，不进入代码或仓库）。

  > ⚠️ 仓库里**不含任何第三方音频**。若你自行放入商业游戏的原版音乐，请只在本机使用——把有版权的素材重新分发到公开仓库是侵权行为。`.gitignore` 已默认忽略 `*.mp3` 等音频格式。
