# 我的题解库 · Tiku

一个 **洛谷风格的个人题解库**：把做过的题目、代码和多种题解存在一起，随时回顾。

单个 HTML 文件，**零外部依赖**，双击就能用。电脑上写、手机上看。

![预览](preview.jpg)

---

## 为什么做这个

做完一道题，题解、踩过的坑、以及不同做法的代码，通常散落在洛谷剪贴板、本地 .cpp 文件和聊天记录里。过两个月想回顾，找不到。

这个工具把「题目 + 多种题解 + 个人笔记」放在一个界面里，并且：

- **一题可以挂多份题解**（比如「二维 DP」和「一维滚动数组」并排对比）
- **中文记谱**、难度徽章、标签，沿用洛谷的习惯
- **数据是你的**：存在本地，可导出 JSON；也可以同步到自己的私有 Gist

---

## 功能

### 题库与题解

- 左侧题库栏按 **洛谷七档难度**配色（入门 → NOI/NOI+/CTSC），支持标签与全文搜索
- 搜索范围包括**标题、来源、标签、题目正文、个人笔记、以及题解代码内容**
- 题目页三个标签页：**题目描述** / **题解(N)** / **我的笔记**
- 样例区输入输出并排，可一键复制
- 代码块支持**一键复制**和**下载 .cpp**（文件名按题号自动生成）
- 内置 **C++ 语法高亮**（预处理器、关键字、STL 类型、字符串、数字、注释、函数调用分别着色）
- 收藏、编辑、复制一份、删除
- **比赛分组**：按「比赛 → 题目」两级浏览，一场比赛自动编号 A/B/C/D，支持批量导入整场

### 比赛分组（两级导航）

可以把一整场比赛收进库里，按「比赛 → 题目」两级浏览：

- 侧栏顶部切换 **题目 / 比赛** 两种模式
- **＋ 新建比赛**：填名称、日期、备注
- 进比赛页可以看到这场的全部题目，**自动编号 A、B、C、D…**（按录入顺序）
- **批量导入**：在比赛页粘贴多个题目链接（一行一个），一次全部抓取并归入本场——这就是「加入一整场比赛」最省事的做法
- 点列表里任意一题进入题目页，头部有比赛面包屑，点「← 返回」回到这场比赛的题目列表
- 全库题目列表里，属于某场比赛的题会直接显示比赛名

删除比赛时**不会连带删除题目**，只会解除归属——不会误删你写好的题解。

比赛本身也走同一套同步规则：带时间戳合并、删除写墓碑，多设备之间不会互相复活。

### 从链接导入题目

粘贴一个题目链接，自动填入**题目名、描述、输入输出格式、样例、时空限制**，代码你自己贴。

| 站点 | 解析方式 |
|---|---|
| 洛谷 | 页面内嵌结构化数据，**难度、标签、时空限制、LaTeX 公式全部保留** |
| Codeforces | `.problem-statement` 结构 |
| AtCoder | `#task-statement` 分节 |
| LibreOJ / 牛客 / 自建 OJ | 通用标题分节 |
| 其他任意站点 | 兜底提取标题与正文 |

**为什么要走代理**：浏览器有同源策略，而洛谷等站点不返回 `Access-Control-Allow-Origin`，页面无法直连。因此采用**多通道并发**（allorigins / codetabs / jina / 直连），取最快返回的那个，其余立即中断。

抓不到时，展开「粘贴页面内容」，在题目页 `Ctrl+A` 复制粘贴即可——**解析逻辑完全相同**，所以这条路径永远可用。

### 云端同步

数据默认存在浏览器 `localStorage`，**不同设备之间不会自动同步**。提供三种方式：

1. **GitHub Gist 双向自动同步** —— 打开时自动拉取、改动后自动上传（防抖 2.5 秒）、切回前台与网络恢复时各同步一次
2. **只读链接** —— 打开时从指定 URL 拉取，只读不写
3. **导出 / 导入 JSON** —— 手动备份与迁移

**手机查看是一条链接的事**：电脑上配好后点「生成手机查看链接」，得到

```
https://你的域名/index.html#sync=gist&id=xxxx&ro=1
```

手机点开**自动配好同步方式并拉取**，不用手填任何东西。令牌绝不放进链接（会留在浏览器历史与 Referer 里）。

**合并规则**：每道题带修改时间戳，同 id 冲突时保留较晚的版本；删除会记录「墓碑」，不会被另一台设备重新拉回来（墓碑 365 天后清理）。

> 手机端免令牌读取需要 **公开 Gist**（私密 Gist 无法匿名读取）。公开意味着凭 Gist ID 任何人都能看，介意就用私密 Gist + 手机填一次令牌。

---

## 快速开始

```bash
git clone https://github.com/<你的用户名>/tiku.git
cd tiku
# 直接双击 index.html，或者：
python3 -m http.server 8000   # 然后浏览器打开 http://localhost:8000
```

没有任何构建步骤、没有 npm install、没有依赖。

### 部署到 GitHub Pages

1. 把仓库推到 GitHub
2. Settings → Pages → Source 选 `main` 分支、`/ (root)` 目录
3. 得到 `https://<你的用户名>.github.io/tiku/`

因为是单个静态文件，Vercel / Netlify 拖拽上传，或腾讯云 COS、阿里云 OSS 都一样能放。

---

## 数据安全

- 数据在**浏览器 localStorage**（上限通常 5 MB，按本项目的结构约可存 1500 道题）
- **清理浏览器数据会连同题库一起清掉** —— 重要题解请定期导出 JSON
- 存储写满时会明确提示，不会静默丢数据
- 删除操作使用「墓碑」标记，跨设备同步时不会被别的设备重新拉回来

---

## 技术说明

| | |
|---|---|
| 形态 | 单文件 HTML，无构建、无依赖、无后端 |
| 体积 | 约 200 KB（含内嵌背景美术） |
| 数据 | `localStorage` + 可选 GitHub Gist 同步 |
| 高亮 | 自研 C++ 语法高亮（不引入 highlight.js 等外部库） |
| 主题 | 亮色 / 暗色跟随系统（`prefers-color-scheme`） |
| 兼容 | 桌面与移动端；无外部资源，微信内置浏览器可直接打开 |

### 测试

```bash
npm install     # 只装 jsdom，用于在真实 DOM 环境里跑测试
npm test
```

测试覆盖：从链接导入的各站点解析器（含真实页面快照）、代理链选路与降级、云端同步与合并算法（双设备模拟）、导出导入往返、C++ 高亮无损性与转义。其中一部分会真实调用 `g++` 编译内置题解并跑样例，机器上没有 `g++` 时会自动跳过。

---

## 授权

**代码：MIT**（见 `LICENSE`）

**背景角色美术：CC BY-NC-SA 4.0**（见 `LICENSE-ARTWORK` 与 `NOTICE`）

本项目 `index.html` 内嵌的虎鲸娘背景立绘来自 DSH 皮肤 **虎鲸链路（ORCA LINK）**，其美术是 **上善** 原创角色「鲸鱼娘」的衍生作品。按 CC BY-NC-SA 4.0 的要求保留完整署名链：

| 版权所有人 | 内容 | 主页 |
|---|---|---|
| 上善 | 鲸鱼娘角色形象原作 | [Pixiv](https://www.pixiv.net/users/62155430) |
| Small-tailqwq | ORCA LINK 皮肤场景与角色素材 | [GitHub](https://github.com/Small-tailqwq) |

> ⚠️ **因此本仓库整体不能称为「开源」**。CC BY-NC-SA 4.0 含**禁止商用**条款，不符合开放源代码定义（OSI 不认可 NC 类协议）。准确说法是：**源码公开，个人与非商业用途免费使用**。
>
> 若需要用于商业场景，或希望整站以 MIT 开源，请移除 `index.html` 中的内嵌美术（搜索 `--bg-art` 即可定位）。

---

## 已知限制

- **难度与标签仅洛谷能自动填入**，其他站点不暴露这两项，需手动选
- **链接导入依赖第三方公共代理**，可能变慢或失效；此时状态栏会给出每个通道的具体错误，并指引改用「粘贴页面内容」
- **解析器依据各站当前页面结构**，站点改版可能失效（有多级降级链兜底）
- **冲突解决是整题级**，不是字段级：同一道题在两端都离线改过，后保存的会整份覆盖


---

## 附带：中国象棋（`xiangqi/`）

仓库里还有一个**单文件中国象棋**：代码零依赖、零构建，只带一个背景音乐文件。

**https://the-hiker0.github.io/tiku/xiangqi/**

- 完整规则：蹩马腿、塞象眼、象不过河、炮翻山、兵卒过河、将帅照面
- **本变体按「吃将定胜负」**：不拦截送将着法，任何着法都能走，走出后对方吃掉你的将即判负
- AI：迭代加深 + Alpha-Beta（PVS）+ 置换表 + 静态搜索 + 着法排序（MVV-LVA / 杀手着法 / 历史启发 / 反制着法）
  + **空着裁剪 + 后着削减（LMR）+ 静态剪枝 + 迭代时间预测**
- **四档难度**，选择「人机对战」后会先弹窗让你挑，之后也能随时从面板改：
  | 档位 | 搜索深度 | 每步思考上限 | 特点 |
  |---|---|---|---|
  | 档位 | 深度上限 | 时间上限 | 实测达到 | 实测用时 | 特点 |
  |---|---|---|---|---|---|
  | 入门 | 3 层 | 0.4 s | 3 层 | <0.1 s | 只算三层，常在几步好棋里随手挑一个 |
  | 普通 | 5 层 | 1.0 s | 5 层 | <0.1 s | 能守住大子，会吃明显的漏子 |
  | 困难 | 7 层 | 2.2 s | 7 层 | 0.07 s | 每步都挑最优，很少失误 |
  | 大师 | 11 层 | 4.0 s | 10~11 层 | 1.2 s | 全力搜索，认真跟你下 |

  > 时间上限只是安全网：实测各档远用不满（12 个开局取平均）。
  > 另附**迭代时间预测**——上一层耗时×4 装不进剩余预算时就不再开新层，
  > 免得"起一个注定搜不完的迭代、然后在超时处被打断"那样白等几秒。
- **任何档位都不会主动送将**：根节点会把「走完对方立刻能吃我将」的着法先筛掉（除非每一步都保不住）
- **内置开局库**：3669 个开局局面、5324 条着法，数据来自 ElephantEye（象棋巫师同源引擎）
  自带的 BOOK.DAT——那是经过长期实战打磨的开局知识，覆盖前 18 手左右
- **时间银行**：开局用库秒回，省下的时间存起来留给中局关键局面（上限为单步预算的 3 倍）。
  没有它，开局省下的时间就白白蒸发了
- **让我少子**：可主动让掉自己的马/车，想让对局更难时用。这是**明确写在界面上**的主动削弱，
  引擎不会偷偷改棋
- 中文记谱（炮二平五 / 前马退三），支持悔棋、双人与人机对战
- 由于取消了「将死」判定，另加两条判和规则：同一局面重复三次、连续 60 回合无吃子
- **背景音乐固定用 `xiangqi/bgm.mp3`**，没有第二个音源（界面里的「换背景音乐」已移除）。文件在就放、不在就静音，面板会明确写出当前状态
- **音效**：落子/吃子音色不同、将军会喊出来（浏览器语音合成）。同目录有 `sfx-*.mp3` 就放文件，没有就用 Web Audio 现场合成兜底
  | 文件名 | 用途 | 是否已入库 |
  |---|---|---|
  | `bgm.mp3` | 背景音乐（唯一音源） | ✅ 已入库 |
  | `sfx-move.mp3` | 落子 | 可选，自己放同目录 |
  | `sfx-capture.mp3` | 吃子 | 可选 |
  | `sfx-check.mp3` | 将军 | 可选 |
  | `sfx-win.mp3` / `sfx-lose.mp3` | 胜利 / 失败 | 可选 |

  > ⚠️ **`xiangqi/bgm.mp3` 是造梦西游（4399）的原版音乐，版权不属于本仓库**，MIT 与 CC BY-NC-SA 都不覆盖它。传到公开仓库是按仓库主的决定做的，fork 或再分发请自行处理版权，详见 `NOTICE`。`.gitignore` 仍然忽略其它所有音频格式，只对这一个文件放行。

棋力说明（全部为实测，不是估计）：

**与外部强引擎的真实对比**（ElephantEye，2004-2016 年间的专业级象棋引擎，
自对弈 10 局、每局交换先后手，本引擎 400ms/步）：

| 对手强度 | 本引擎得分率 |
|---|---|
| ElephantEye 深度 4 | 0.350 |
| ElephantEye 深度 6 | 0.250 |
| ElephantEye 深度 8 | 0.100 |

即**本引擎当前不如 ElephantEye 的深度 4**。1114 手对局中，本引擎在标准规则下
**0 手违规**（说明规则实现是可靠的）。诚实结论：这是**业余棋手水平**，
不是职业水平——职业级引擎（Pikafish 等）是 C++ + NNUE + 多线程，量级完全不同。

**自对弈 A/B 实测（等时，每局交换先后手）：**

- 相比上一版，**搜索技术改进带来 +55 Elo**（400 局，95% 区间 [+21, +90]，显著）；
  在 500 ms 等时下复测为 **+51 Elo**（200 局，区间 [+3, +101]，显著）。
- 同深度节点数下降约 **7 倍**，因此四档的深度上限从 2/4/6/8 提到 3/5/7/11：
  **「大师」档从 2.8 秒搜到 7.7 层，变成 1.2 秒搜到 10.8 层**（更深且更快）。
- **试过但放弃的改动**：给评估函数加机动性 / 将危险 / 兵结构 / 车通线四个分项，
  在等时条件下实测**显著变弱**（−67 ~ −117 Elo）。原因是评估变复杂导致搜索变浅，
  收益抵不过代价。该结论已写进代码注释，避免以后重复踩坑。

引擎是「业余爱好者」水平，不是商业引擎，但不再会送子、也不会把赢棋走成和棋。

> 走法生成用标准 perft 数据校验过：perft(1)=44、perft(2)=1920、perft(3)=79666，与公开值完全一致。
